...
exports.HyperlendSDK = void 0;
const ethers_1 = require("ethers5");
const types_1 = require("./types");
const types_2 = require("./types");
class HyperlendSDK {
    constructor(providerOrSigner, registryAddress) {
        this.providerOrSigner = providerOrSigner;
        const factory = new types_1.HyperlendPairRegistry__factory();
        this.registry = factory.attach(registryAddress);
        if (ethers_1.ethers.Signer.isSigner(providerOrSigner)) {
     ...
     */
    getTokenContract(tokenAddress, includeApprovalMethods = false) {
        const methods = [
        ...
        if (includeApprovalMethods) {
            methods.push("function allowance(address owner, address spender) external view returns (uint256)", "function approve(address spender, uint256 amount) external returns (bool)");
        }
        const contract = new ethers_1.ethers.Contract(tokenAddress, methods, this.providerOrSigner);
        if (process.env.SDK_DEBUG === '1' && (!contract.provider || contract.provider === null)) {
            console.warn(`[SDK DEBUG] getTokenContract: Contract at ${tokenAddress} has null provider`);
        }
        return contract;
    }
    /**
    ...
    async getSignerAddress() {
        if (!ethers_1.ethers.Signer.isSigner(this.providerOrSigner)) {
     ...
     */
    async approveTokenIfNeeded(tokenAddress, spender, amount, autoApprove) {
     ...
     */
    async estimateGasWithBuffer(transaction, bufferMultiplier = 1.2) {
        try {
    ...
    handleTransactionError(error) {
        if (error.code === ethers_1.ethers.errors.CALL_EXCEPTION) {
     ...
     */
    async getPairContract(pairAddress) {
        try {
            const factory = new types_2.HyperlendPair__factory();
            const pair = factory.attach(pairAddress);
            // Always connect to providerOrSigner to ensure provider is set (hotfix for null provider)
            const connectedPair = pair.connect(this.providerOrSigner);
            if (process.env.SDK_DEBUG === '1') {
                console.log(`[SDK DEBUG] getPairContract: connectedPair.provider is ${connectedPair.provider ? 'SET' : 'NULL'}, providerOrSigner type: ${typeof this.providerOrSigner?.call}`);
            }
    ...
    async setDeployers(deployers, allow, signer, overrides) {
        if (!ethers_1.ethers.Signer.isSigner(signer)) {
    ...
    async getPairAddressByName(name) {
        if (!name) {
     ...
     */
    async readPairData(pairAddress) {
        try {
            const pair = await this.getPairContract(pairAddress);
            const [asset, collateral, maxLTV, cleanLiquidationFee, dirtyLiquidationFee, protocolLiquidationFee, totalAssetAmount, totalBorrowAmount, totalCollateral, currentRateInfo,] = await Promise.all([
                pair.asset(),
     ...
     */
    async readUserPosition(pairAddress, userAddress) {
        try {
            const pair = await this.getPairContract(pairAddress);
            ...
            const [userCollateralBalance, userBorrowShares, exchangeRateInfo,] = await Promise.all([
                pair.userCollateralBalance(userAddress),
                pair.userBorrowShares(userAddress),
                pair.exchangeRateInfo(),
            ]);
            const liquidationPrice = userBorrowShares.gt(0) && !userCollateralBalance.isZero()
    ...
    // ===============================
    async getTotalAsset(pairAddress) {
        try {
            const pair = await this.getPairContract(pairAddress);
    ...
    async getTotalBorrow(pairAddress) {
        try {
            const pair = await this.getPairContract(pairAddress);
     ...
     */
    async supply(pairAddress, amount, userAddress, autoApprove = true) {
        try {
            const pair = await this.getPairContract(pairAddress);
            const assetTokenAddress = await pair.asset();
            const assetInfo = await this.getTokenInfo(assetTokenAddress);
            const approvalResult = await this.approveTokenIfNeeded(assetTokenAddress, pairAddress, amount, autoApprove);
            const gasLimit = await this.estimateGasWithBuffer(() => pair.estimateGas.deposit(amount, userAddress));
     ...
     */
    async borrow(pairAddress, amount, collateralAmount, userAddress, options) {
        var _a;
        try {
            if (amount.lte(0))
            ...
            if (collateralAmount.gt(0)) {
                approvalResult = await this.approveTokenIfNeeded(collateralTokenAddress, pairAddress, collateralAmount, (_a = options === null || options === void 0 ? void 0 : options.autoApprove) !== null && _a !== void 0 ? _a : true);
            }
            ...
            const oracle = new ethers_1.ethers.Contract(oracleAddress, ["function getPrices() external view returns (bool _isBadData, uint256 _priceLow, uint256 _priceHigh)"], this.providerOrSigner);
            ...
            const gasLimit = (options === null || options === void 0 ? void 0 : options.gasLimit) || await this.estimateGasWithBuffer(() => pair.estimateGas.borrowAsset(amount, collateralAmount, userAddress), 1.5 // 50% buffer for borrow operations
            );
     ...
     * @param userAddress Address of the user.
     */
    async withdraw(pairAddress, shares, userAddress) {
        try {
            if (shares.lte(0))
            ...
            const assetTokenAddress = await pair.asset();
            const assetInfo = await this.getTokenInfo(assetTokenAddress);
            const gasLimit = await this.estimateGasWithBuffer(() => pair.estimateGas.withdraw(shares, userAddress, userAddress));
     ...
     * @param autoApprove Whether to automatically approve tokens if allowance is insufficient.
     */
    async repay(pairAddress, shares, userAddress, autoApprove = true) {
        try {
            if (shares.lte(0))
            ...
            const gasLimit = await this.estimateGasWithBuffer(() => pair.estimateGas.repayAsset(shares, userAddress), 1.5 // 50% buffer for repay operations
            );
     ...
     * @param autoApprove Whether to automatically approve tokens if allowance is insufficient.
     */
    async addCollateral(pairAddress, amount, userAddress, autoApprove = true) {
        try {
            if (amount.lte(0))
            ...
            const gasLimit = await this.estimateGasWithBuffer(() => pair.estimateGas.addCollateral(amount, userAddress));
     ...
     * @param userAddress Address of the user.
     */
    async removeCollateral(pairAddress, amount, userAddress) {
        try {
            if (amount.lte(0))
                throw new Error("Collateral amount must be greater than zero");
            const pair = await this.getPairContract(pairAddress);
            const collateralTokenAddress = await pair.collateralContract();
            const collateralInfo = await this.getTokenInfo(collateralTokenAddress);
            const gasLimit = await this.estimateGasWithBuffer(() => pair.estimateGas.removeCollateral(amount, userAddress));
...
exports.HyperlendSDK = HyperlendSDK;
